...
import { type Driver, type Order } from "@/lib/los-nietos-data";
import { useDrivers, useOrders } from "@/lib/orders-store";
...
function DashboardSection() {
  ...
  const metrics = useMemo(() => {
    ...
    return {
      ...
      customers: new Set(orders.map((o) => o.customer)).size,
      margin,
    };
  }, [products, orders]);
  ...
  return (
    <>
      <section className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <MetricCard
          label="Margen"
          value={`$${metrics.margin.toLocaleString()}`}
          delta="+15%"
      ...
      <section className="grid md:grid-cols-2 gap-5">
        ...
        <div className="bg-card rounded-2xl p-5 shadow-card flex flex-col">
          <h2 className="font-display text-lg mb-2">
            Análisis IA <span className="text-xs font-normal text-muted-foreground">(en vivo)</span>
          </h2>
...
function AiModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    ...
    >
      <div
        className="bg-card rounded-2xl w-full max-w-md p-6 animate-modal max-h-[85vh] overflow-y-auto"
      ...
      >
        ...
        {error && (
          <div
            ...
            style={{ background: "var(--soft-red)", color: "oklch(0.45 0.18 25)" }}
          >
              ...
              {data.recommendations.map((r, i) => (
                <div
                  ...
                  style={{
                    ...
                    borderLeft: `4px solid ${toneMap[r.tone] ?? "var(--brand-blue)"}`,
                  }}
                >
  ...
  return (
    <section className="bg-card rounded-2xl p-5 shadow-card">
      <div className="flex items-center justify-between mb-4">
        ...
        <button
          onClick={openNew}
          className="rounded-xl px-4 py-2 text-sm font-semibold text-white bg-gradient-hero"
        >
          + Nuevo
        </button>
      </div>
          ...
          {products.map((p) => (
            <li key={p.id} className="py-3 flex items-center justify-between gap-2">
              ...
              <div className="flex gap-1">
                <button
                  onClick={() => openEdit(p)}
                  className="text-xs px-3 py-1.5 rounded-lg bg-muted font-semibold"
                >
                  Editar
                </button>
                <button
                  onClick={() => {
                    if (confirm(`¿Borrar ${p.name}?`)) delMut.mutate(p.id);
                  }}
      ...
      {open && (
        ...
        >
          <form
            onSubmit={submit}
            className="bg-card rounded-2xl w-full max-w-md p-6 animate-modal space-y-3"
            style={{ borderTop: "4px solid var(--brand-blue)" }}
            onClick={(e) => e.stopPropagation()}
          >
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <Field label="Ventas">
                <NumberInput value={form.sales} onChange={(v) => setForm({ ...form, sales: v })} />
              </Field>
            </div>
            ...
            <div className="flex gap-2 pt-2">
              <button
                type="button"
                onClick={() => setOpen(false)}
                className="flex-1 rounded-xl py-2.5 font-semibold bg-muted"
              >
                Cancelar
              </button>
              <button
                ...
                className="flex-1 rounded-xl py-2.5 font-semibold text-white bg-gradient-green disabled:opacity-50"
              >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-2 mb-4">
        {drivers.map((d) => {
          const count = orders.filter(
            (o: Order) => o.driverCode === d.code && o.status !== "entregada",
          ).length;
          return (
            <button
              key={d.code}
              onClick={() => setDriverFilter(d.code)}
              className={`rounded-xl p-2 text-left border-2 transition-colors ${driverFilter === d.code ? "border-primary bg-accent" : "border-transparent bg-muted"}`}
            >
      ...
      <div className="flex gap-2 bg-muted rounded-xl p-1 mb-3">
        {(["pendientes", "entregadas"] as const).map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
            className={`flex-1 py-2 rounded-lg text-sm font-semibold capitalize transition-colors ${tab === t ? "bg-card shadow-sm" : "text-muted-foreground"}`}
          >
...
function OrderRow({ order }: { order: Order }) {
  const drivers = useDrivers();
  const [open, setOpen] = useState(false);
  const driver = drivers.find((d: Driver) => d.code === order.driverCode);
            ...
            <div>
              ...
              <div className="font-semibold">
                ...
                <span
                  ...
                  style={{ background: order.paid ? "var(--brand-green)" : "var(--brand-orange)" }}
                >
